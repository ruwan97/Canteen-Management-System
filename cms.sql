...
--
-- Host: 127.0.0.1:3306
-- Generation Time: Jul 04, 2021 at 12:51 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `ad_user_account_create_procedure` (IN `id` VARCHAR(6), IN `name` VARCHAR(255), IN `email` VARCHAR(100), IN `role` VARCHAR(8))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `ad_user_account_update_procedure` (IN `id` VARCHAR(6), IN `name` VARCHAR(255), IN `email` VARCHAR(100), IN `role` VARCHAR(8))  BEGIN
UPDATE `user` 
SET 
`user_id` = id, 
`user_name` = name, 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `user_account_create_procedure` (IN `id` VARCHAR(6), IN `name` VARCHAR(255), IN `email` VARCHAR(100), IN `password` VARCHAR(50), IN `role` VARCHAR(8), IN `contact` INT(10), IN `image` VARCHAR(255))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `user_account_update_procedure` (IN `id` VARCHAR(6), IN `name` VARCHAR(255), IN `email` VARCHAR(100), IN `password` VARCHAR(50), IN `role` VARCHAR(8), IN `contact` INT(10), IN `image` VARCHAR(255))  BEGIN
UPDATE `user` 
SET 
`user_id` = id, 
`user_name` = name, 
...
--
-- Stand-in structure for view `ad_view_items`
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `ad_view_items`;
CREATE TABLE IF NOT EXISTS `ad_view_items` (
`item_id` varchar(6)
,`item_name` varchar(255)
,`unit_price` float
,`quantity` int(5)
...
('itm003', 'roll', 25, 'images/roll.png'),
('itm002', 'patis', 10, 'images/patis.png'),
('itm001', 'bun', 15, 'images/bun.png'),
('itm004', 'eggs', 22, 'images/egg.png'),
('itm005', 'bread', 55, 'images/bread.png'),
('itm006', 'buiscuit', 45, 'images/buiscuit.png'),
('itm007', 'soap', 48, 'images/soap.png');
...
(23, 'TG0001', 'itm005', 4, 220, '2021-06-30 09:29:43'),
(22, 'TG0002', 'itm003', 3, 75, '2021-06-30 09:04:17'),
(25, 'TG0002', 'itm004', 2, 44, '2021-06-30 09:30:24'),
(26, 'TG0003', 'itm005', 2, 110, '2021-06-30 09:30:54'),
(28, 'TG0005', 'itm002', 5, 50, '2021-06-30 22:56:33'),
(30, 'TG0004', 'itm003', 7, 175, '2021-07-02 19:03:00'),
(31, 'TG0001', 'itm001', 13, 195, '2021-07-02 20:08:04');
...
CREATE TABLE IF NOT EXISTS `user` (
  ...
  `user_role` varchar(8) DEFAULT NULL COMMENT 'admin = 1 , \r\nuser = 2',
...
('TG0002', 'damith', 'damith@gmail.com', '12345', '2', 758478542, '74337060_2346520482141651_4855466270499274752_n.jpg'),
('adm001', 'kamal', 'kamal@gmail.com', '12345', '1', 712365478, '67083064_105075834150769_6716344410080018432_n.jpg'),
('TG0001', 'kalana', 'kalana@gmail.com', '12345', '2', 712563457, '74337060_2346520482141651_4855466270499274752_n.jpg');
...
DROP VIEW IF EXISTS `ad_view_users`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `ad_view_users`  AS  select `user`.`user_id` AS `user_id`,`user`.`user_name` AS `user_name`,`user`.`user_email` AS `user_email`,`user`.`user_role` AS `user_role`,`user`.`user_contact` AS `user_contact`,`user`.`user_image` AS `user_image` from `user` ;
...
DROP VIEW IF EXISTS `user_view_items`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `user_view_items`  AS  select `itm`.`item_name` AS `item_name`,`itm`.`unit_price` AS `unit_price`,`inv`.`quantity` AS `quantity`,`itm`.`item_image` AS `item_image` from (`item` `itm` join `inventory` `inv`) where (`itm`.`item_id` = `inv`.`item_id`) ;
